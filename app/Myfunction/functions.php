...
function Select_DonviTinh($data, $select = 0)
{
    foreach ($data as $val) {
        $id = $val['id'];
        $name = $val['name'];
        $soluong = $val['soluong'];
        if ($select != 0 && $soluong == $select) {
            echo "<option value='$soluong' selected = 'true'>$name</option>";
        } else {
            echo "<option value='$soluong'>$name</option>";
        }
    }
}
